# Hi, I'm Tiernan (T-Lind)!


<div style="display: flex; justify-content: center;">
  <img src="angularized_300.png" style="height:190px" alt="T-Lind" />
</div>


Welcome to my GitHub! I'm a dual-degree student at Texas A&M University, pursuing two BS degrees--**Computer Science** and **Physics** (3.84 GPA). With experience in **machine learning**, **AI**, and **vector search technologies**, I’m passionate about pushing the boundaries of machine intelligence.

A quick few things about me:
* Founded an educational ML/AI startup ($4M valuation) as a student
* Computer Science & Physics dual degree (full ride to Texas A&M, 1540 SAT)
* I have a [patent pending](https://patents.google.com/patent/US20240320487A1/) for a novel RL training architecture called REPNET to improve efficiency in specific RL applications
* Blue Origin Intern (Advanced Concepts; Aerospace Software & ML)
* Worked at MITRE designing ML algorithms and simulation software for missile interception; "outstanding" performance review


Check out my website at [t-lind.github.io](https://t-lind.github.io/) for more information. Trust me, it's worth it.

<a href="https://git.io/streak-stats"><img src="https://streak-stats.demolab.com?user=T-Lind" alt="GitHub Streak" /></a>
